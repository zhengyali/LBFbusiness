...
<div class="body-box" style="float:right">
	<form id="jvForm" action="edit.do" method="post">
		<input type="hidden" name="id" value="${brand.id}"/>  <%--id作为隐藏域进行绑定pojo--%>
		<input type="hidden" name="tname" value="${name}"/>
		<input type="hidden" name="tisDisplay" value="${name}"/>
		<input type="hidden" name="tpageNo" value="${pageNo}"/>
			...
			<tbody>
				...
				<tr>
					...
					<td width="80%" class="pn-fcontent">
						...
						<input type="hidden" name="imgUrl" id="imgUrl" value="${brand.imgUrl }"/> <%--这个隐藏域用于图片路径的提交--%>
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<input type="text" class="required" name="description" value="${brand.description}" maxlength="255"  size="60"/>
					</td>
				</tr>
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
