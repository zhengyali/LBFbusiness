...
名称: <input type="text" name="name"/>
	<select name="brandId">
		...
		<c:forEach  items="${brands}" var="brand">  <%--cif 标签做回显--%>
			<option value="${brand.id}" <c:if test="${brandId == brand.id}">selected="selected" </c:if>> ${brand.name}</option>
		</c:forEach>

	</select>
	<select name="isShow"><%--isShow 是Boolean类型的，springmvc会将0 和 1自动转为false 和 true ，默认是下架    --%>
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" class="pn-ltable">
	...
	<c:forEach items="${pagination.list }" var="product">
		<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
			...
			<td align="center"><img width="50" height="50" src="${product.images[0] }"/></td><%--图片为图片集--%>
